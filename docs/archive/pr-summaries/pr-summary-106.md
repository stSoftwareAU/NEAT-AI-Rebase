...
* The new gate ran on this PR and reported its no-op branch live (run
  35154071814, job **Auto-increment Crate Version**, pass):

  ```text
  auto-version.sh: already ahead of the base branch (0.1.0 -> 0.1.1) — no bump needed
  Version is already ahead of the base branch — nothing to commit
  ```

* All PR checks pass on the pushed branch, the new job included
  (`gh pr checks 114 --watch --fail-fast`, exit 0).

Two behaviours cannot be exercised before the merge, and are stated as such:
the bump commit CI pushes onto a PR branch whose version is *level* with the
base, and the tag/release cut on the push to `Develop`. Both are covered by the
trigger/path tests plus the script tests below.
...
- **met** — tests and quality checks pass — evidence: `./quality.sh` green after the final edit, and every CI check on PR #114 passes; the Spec reviewer independently ran shellcheck, actionlint, fmt, clippy, the full test suite and markdownlint — reviewer: met
